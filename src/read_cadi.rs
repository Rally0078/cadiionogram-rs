...
pub struct Metadata {
    ...
    pub time_partitions: HashMap<String, usize>,
...
pub struct CadiData {
    ...
    pub complex_signal: Vec<i8>, // Flattened: [dopbin][receiver_re_im]
}
...
impl MDReader {
    ...
    fn convert_bins_to_vals(
        ...
        npulses_avgd: u8,
        pps: u8,
    ) -> (Vec<f32>, Vec<f32>, Vec<f32>, Vec<i8>) {
        let n = dopbin_iq.len();
        if n == 0 {
        ...
        for i in 0..n {
            ...
            complex_signal.extend(dopbin_iq[i].iter().map(|&v| v as i8));
        }
...
impl<'a> ReaderContext<'a> {
    ...
    fn read_header(&mut self) -> Result<(), ()> {
        ...
        self.metadata.datetime = self.read_string(22).map_err(|_| self.mark_header_incomplete())?;
    ...
    fn read_records(&mut self, obs: &mut ObservationBuffer) -> Result<(), ()> {
        ...
        let hour = self.metadata.datetime.get(8..10).and_then(|s| s.parse::<u8>().ok()).unwrap_or(0);
